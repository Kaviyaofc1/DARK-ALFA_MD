    ...
    if (args[0] === "on") {
      if (!checkdata) {
        ...
        Miku.sendMessage(
          m.from,
          {
            text: `*ƊƋГƘ ѦしẜƋ ⱮƊ* has been Re-Activated in this group!`,
        ...
        if (checkdata.botSwitch == "true")
          return Miku.sendMessage(
            m.from,
            { text: `*ƊƋГƘ ѦしẜƋ ⱮƊ* is already Activated in this group !` },
            { quoted: m }
          );
        await mk.updateOne({ id: m.from }, { botSwitch: "true" });
        return Miku.sendMessage(
          m.from,
          { text: `*ƊƋГƘ ѦしẜƋ ⱮƊ* has been Activated in this group! Now everyone here can use bot.` },
          { quoted: m }
        );
      }
    } else if (args[0] === "off") {
      if (!checkdata) {
        ...
        return Miku.sendMessage(
          m.from,
          {
            text: `*ƊƋГƘ ѦしẜƋ ⱮƊ* has been De-Activated in this group !\n\nNow only *Admins* can use bot`,
          },
          { quoted: m }
        );
      } else {
        if (checkdata.botSwitch == "false")
          return Miku.sendMessage(
            m.from,
            { text: `*ƊƋГƘ ѦしẜƋ ⱮƊ* is already De-Activated in this group !\n\nNow only *Admins* can use bot` },
            { quoted: m }
          );
